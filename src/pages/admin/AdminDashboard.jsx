...
import { Users, DollarSign, TrendingUp, Plus, Settings, CreditCard, Shield, Copy, CheckCircle } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const load = () => getToken().then(token => fetch('/api/admin?action=stats', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).then(d => { setStats(d); setLoading(false) }).catch(() => setLoading(false)))
  useEffect(() => { load() }, [])
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-semibold text-slate-900">Admin Portal</h1><p className="text-sm text-slate-400 mt-0.5">TutorMastery Platform — H2K Group</p></div>
        <button onClick={() => setShowAddTutor(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: 'var(--honey)', color: 'var(--navy)' }}><Plus size={14} /> Add Tutor</button>
      </div>
      ...
      {tab === 'tutors' && (
        <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
          ...
          {loading ? <div className="p-8 text-center text-sm text-slate-400">Loading…</div> : !stats?.tutors?.length ? <div className="p-8 text-center text-sm text-slate-400">No tutors yet</div> : (
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100 bg-slate-50">{['Tutor','Subjects','Students','Sessions','Fee','Payment','Status',''].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">{h}</th>)}</tr></thead>
              <tbody>
                {stats.tutors.map(t=>(
                  <tr key={t.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3"><div className="font-semibold text-slate-800">{t.firstName} {t.lastName}</div><div className="text-xs text-slate-400">{t.email}</div></td>
                    ...
                    <td className="px-4 py-3"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${t.feePercent===0?'bg-teal-100 text-teal-800':'bg-amber-100 text-amber-800'}`}>{t.feePercent===0?'Free (beta)':`${t.feePercent}%`}</span></td>
                    <td className="px-4 py-3"><span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{t.paymentMethod==='platform'?'Platform':t.paymentMethod==='stripe'?'Own Stripe':'PayID'}</span></td>
                    <td className="px-4 py-3"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${t.status==='active'?'bg-teal-100 text-teal-800':'bg-red-100 text-red-700'}`}>{t.status}</span></td>
                    <td className="px-4 py-3"><TutorFeeEditor tutor={t} getToken={getToken} onSaved={msg=>{showToast(msg);load()}} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'bank' && <BankSettings getToken={getToken} initial={stats?.bankSettings} onSaved={msg=>{showToast(msg);load()}} />}
      ...
      {showAddTutor && <AddTutorModal getToken={getToken} onClose={() => setShowAddTutor(false)} onSaved={msg=>{showToast(msg);setShowAddTutor(false);load()}} />}
      {toast && <div className="fixed bottom-6 right-6 bg-slate-900 text-white text-sm font-medium px-4 py-3 rounded-xl shadow-xl z-50 max-w-sm">{toast}</div>}
    </div>
  )
}
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="bg-white rounded-xl border border-slate-100 p-6">
        ...
        <p className="text-xs text-slate-400 mb-4">Used for tutor payouts and parent payments via platform account</p>
        <div className="space-y-3">
          ...
          <div><label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">PayID</label><input className={inp} value={form.payId} onChange={e=>setForm({...form,payId:e.target.value})} placeholder="payments@datamastery.com.au" /></div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-6">
        ...
        <p className="text-xs text-slate-400 mb-4">Copy details to share with tutors or paste into emails</p>
        {form.payId||form.bsb ? (
          <div className="space-y-3">
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="bg-white rounded-xl border border-slate-100 p-6">
        ...
        <p className="text-xs text-slate-400 mb-4">Grant admin access by email. If they haven't signed in yet, access is granted on their first login.</p>
        <div className="space-y-3">
          <div><label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Email Address</label><input className={`${inp} w-full`} value={email} onChange={e=>setEmail(e.target.value)} placeholder="kurahemish@gmail.com" type="email" /></div>
...
function TutorFeeEditor({ tutor, getToken, onSaved }) {
  const [editing, setEditing] = useState(false); const [fee, setFee] = useState(tutor.feePercent); const [saving, setSaving] = useState(false)
  async function save() { setSaving(true); const token = await getToken(); await fetch('/api/admin?action=tutor-settings', { method:'POST', headers:{ Authorization:`Bearer ${token}`, 'Content-Type':'application/json' }, body: JSON.stringify({ tutorId: tutor.id, feePercent: parseFloat(fee) }) }); setSaving(false); setEditing(false); onSaved(`Fee updated to ${fee}% for ${tutor.firstName}`) }
  if (!editing) return <button onClick={()=>setEditing(true)} className="text-xs text-teal-600 hover:underline font-semibold">Edit fee</button>
  return <div className="flex items-center gap-1"><input type="number" value={fee} onChange={e=>setFee(e.target.value)} className="w-14 border border-slate-200 rounded px-1.5 py-1 text-xs" min="0" max="20" step="0.5" /><span className="text-xs text-slate-400">%</span><button onClick={save} disabled={saving} className="text-xs bg-teal-500 text-white px-2 py-1 rounded font-semibold">{saving?'…':'✓'}</button><button onClick={()=>setEditing(false)} className="text-xs text-slate-400">✕</button></div>
}
...
function AddTutorModal({ getToken, onClose, onSaved }) {
  ...
  async function save() { setSaving(true); const token = await getToken(); const res = await fetch('/api/admin?action=tutors', { method:'POST', headers:{ Authorization:`Bearer ${token}`, 'Content-Type':'application/json' }, body: JSON.stringify({ ...form, subjects: form.subjects.split(',').map(s=>s.trim()).filter(Boolean) }) }); const data = await res.json(); setSaving(false); if (res.ok) onSaved(`${form.firstName} ${form.lastName} added ✓`); else alert(data.error) }
